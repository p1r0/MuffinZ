...
class Bs_Plogger_Logger
{
     ...
     * @return Logger
     */
    public function __construct($id = "Anonymous Logger", $fileName = "")
    {
        $this->fileName = APPLICATION_PATH."/logs/logger.log";
        $this->outputFormatter = new Bs_Plogger_Output_Formater();
     ...
     * self::LOG_ALL : Logs everything sent to logger<br>
     * self::LOG_WARNING : Logs errors plus warnings<br>
     * self::LOG_ERROR : Logs only errors<br>
     * self::LOG_DEBUG : Logs errors, warning and debug messages.<br>
     * self::LOG_NONE : No output is logged.  
     * 
    ...
    public function debug()
    {
        ...
        if($this->getLogLevel() >= self::$LOG_DEBUG)
        {
    ...
    public function error()
    {
        ...
        if($this->getLogLevel() >= self::$LOG_ERRORS)
        {
            $msg = "[ERROR]";
            for($i = 0; $i < $numargs; $i++)
            {
    ...
    public function warning()
    {
        ...
        if($this->getLogLevel() >= self::$LOG_WARNINGS)
        {
            $msg = "[WARNING]";
            for($i = 0; $i < $numargs; $i++)
            {
    ...
    public function setOutputFormatter(Bs_Plogger_Output_Formater $of)
    {
